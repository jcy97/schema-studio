// components/dialogs/NewFileDialog.tsx
...
interface NewFileDialogProps {
  ...
  onFileLoaded: (fileName: string, fileData: any) => void;
}
...
const NewFileDialog: React.FC<NewFileDialogProps> = ({
  ...
  const { nodes, relationships } = useSchema();
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // 파일명 추출 (확장자 제외)
      const fileName = selectedFile.name.replace(/\.scst$/i, "");

      // 파일 불러오기 완료 알림
      onFileLoaded(fileName, schemaFile);
      onClose();
    } catch (err) {
      setError(
  ...
  const handleFileDrop = async (e: React.DragEvent<HTMLDivElement>) => {
    ...
    try {
      ...
      // 파일명 추출 (확장자 제외)
      const fileName = droppedFile.name.replace(/\.scst$/i, "");
